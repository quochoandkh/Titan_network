...
read -p "Nhập đường dẫn lưu trữ dữ liệu cho node trên máy chủ (bỏ qua để sử dụng đường dẫn mặc định titan_storage_05): " custom_storage_path
...
if [ -z "$custom_storage_path" ]; then
    ...
    storage_path="$PWD/titan_storage_05"
else
...
container_id=$(docker run -d --restart always -v "$storage_path:/root/.titanedge/titan_05" --name "titan_05" --net=host  nezha123/titan-edge:1.7)

echo "Node titan_05 đã được khởi động với Container ID: $container_id"
...
docker exec $container_id bash -c "\
    ...
    echo 'Node titan_05 đã được cấu hình với dung lượng lưu trữ $storage_gb GB, cổng RPC là $start_rpc_port'"
...
echo "Node titan_05 đã được liên kết."

echo "============================== Node titan_05 đã được cài đặt và khởi chạy thành công ==================================="
}
...
function uninstall_node() {
    ...
    case "$response" in
        [yY][eE][sS]|[yY]) 
            ...
            sudo docker stop "titan_05" && sudo docker rm "titan_05"
            rm -rf "$PWD/titan_storage_05"
