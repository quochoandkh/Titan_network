...
read -p "Nhập đường dẫn lưu trữ dữ liệu cho node trên máy chủ (bỏ qua để sử dụng đường dẫn mặc định titan_storage_04): " custom_storage_path
...
if [ -z "$custom_storage_path" ]; then
    ...
    storage_path="$PWD/titan_storage_04"
else
...
container_id=$(docker run -d --restart always -v "$storage_path:/root/.titanedge/titan_04" --name "titan_04" --net=host  nezha123/titan-edge:1.7)

echo "Node titan_04 đã được khởi động với Container ID: $container_id"
...
docker exec $container_id bash -c "\
    ...
    echo 'Node titan_04 đã được cấu hình với dung lượng lưu trữ $storage_gb GB, cổng RPC là $start_rpc_port'"
...
echo "Node titan_04 đã được liên kết."

echo "============================== Node titan_04 đã được cài đặt và khởi chạy thành công ==================================="
}
...
function uninstall_node() {
    ...
    case "$response" in
        [yY][eE][sS]|[yY]) 
            ...
            sudo docker stop "titan_04" && sudo docker rm "titan_04"
            rm -rf "$PWD/titan_storage_04"
